...
```mermaid
flowchart TB
  subgraph Host["Host NanoClaw process"]
    Channels["Channel adapters<br/>Feishu, Telegram, Discord, CLI"]
    Router["Router and trigger policy"]
    Scheduler["Scheduler"]
    TenantLoader["Tenant config loader"]
    RuntimeClient["Runtime client"]
    ToolWorkers["Host tool workers"]
    CredentialProxy["Credential proxy<br/>scoped provider auth"]
    StatusApi["Reporter and status APIs"]
    CentralDb[("Central host DB<br/>messages, tasks, cursors, groups")]
    Policy["Host policy<br/>sender, approval, mount allowlists"]
  end

  subgraph TenantRepo["Tenant and platform config sources"]
    PlatformSkills["Platform builtin skills"]
    TenantConfig["tenant.json"]
    AgentConfig["agents/{agent}/agent.json"]
    TenantSkills["tenant skills"]
    AgentSkills["agent-local skills"]
  end

  subgraph AgentFinance["Docker service: nanoclaw-agent-finance"]
    Supervisor["Supervisor<br/>JSON-RPC over Unix socket"]
    Manifest["/workspace/agent/skills.manifest.json"]
    SkillMounts["Read-only skill mounts<br/>builtin, tenant, agent"]
    ProviderAdapters["Provider adapters<br/>Claude, OpenCode, mock"]

    subgraph GroupMain["Group: feishu-main"]
      UserMain["Linux user: ncg_feishu_main"]
      LiveMain["Live agent-runner"]
      TaskMain["Isolated task runners"]
      RuntimeMain[("runtime DBs<br/>inbound, outbound, state, tools")]
      GeneratedMain["generated skills<br/>group writable"]
    end

    subgraph GroupOps["Group: ops"]
      UserOps["Linux user: ncg_ops"]
      LiveOps["Live agent-runner"]
      RuntimeOps[("runtime DBs<br/>inbound, outbound, state, tools")]
      GeneratedOps["generated skills<br/>group writable"]
    end
  end
  ...
  Router --> CentralDb
  Scheduler --> CentralDb
  TenantConfig --> TenantLoader
  AgentConfig --> TenantLoader
  TenantLoader --> TenantRepo
  TenantLoader --> RuntimeClient
  StatusApi --> CentralDb
  StatusApi --> RuntimeClient
  RuntimeClient --> Supervisor
  ToolWorkers --> CentralDb
  ToolWorkers --> RuntimeMain
  ToolWorkers --> RuntimeOps
  CredentialProxy --> ProviderAdapters
  Policy --> Router
  Policy --> ToolWorkers
  PlatformSkills --> SkillMounts
  TenantSkills --> SkillMounts
  AgentSkills --> SkillMounts
  ...
  UserMain --> TaskMain
  UserOps --> LiveOps
  LiveMain --> RuntimeMain
  TaskMain --> RuntimeMain
  LiveOps --> RuntimeOps
  LiveMain --> ProviderAdapters
  LiveOps --> ProviderAdapters
  GeneratedMain --> LiveMain
  GeneratedOps --> LiveOps
  RuntimeClient --> RuntimeMain
  RuntimeClient --> RuntimeOps
```
